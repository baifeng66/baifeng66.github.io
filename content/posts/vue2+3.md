...
- [vue2 安装页面](https://v2.cn.vuejs.org/v2/guide/installation.html)
- 所有代码只保留核心代码！！！
...
可以搭配 watch,computed 使用

```html
delBugCallBack(id){ this.bugList = this.bugList.filter(bug=>{ return bug.id !==
id }) }
```
...
不过现在官方推荐使用 Vite 去搭建 Vue 程序~ [Vite 官网](https://cn.vite.dev/)
...
3. 接收时添加类型限制，必要性限制，默认值
   ...
   ```html
   props:{ name: { type: String, required: true }, age: { type: Number, default:
   18 } }
   ```
